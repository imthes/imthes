...
class DataAdapter {
  ...
  loadLocalState() {
    try {
        ...
        if (stored) {
            ...
            if (!data.chat) data.chat = def.chat;
  ...
  // Chat Reward Logic
  async chatClaim() {
      // Mock logic for offline/demo
      const state = { ...this.state };
      const now = Math.floor(Date.now() / 1000);
      if (now - state.chat.lastRewardTs >= state.chat.cooldownSec) {
          state.chat.lastRewardTs = now;
          const reward = 50;
          state.balance += reward;
          triggerBoostAnimation(reward);
          this.saveLocalState(state);
...
let lastRenderedShopHash = '';
let lastRenderedTaskHash = '';
...
function handleTap(e) {
    ...
    if (currentState.energy > 0) {
        ...
        if (currentState.isPremium) gain *= 2;
...
function render() {
  ...
  if (chatStatus && chatBtn) {
      const elapsed = now - (currentState.chat.lastRewardTs || 0);
      const cooldown = currentState.chat.cooldownSec || 60;

      if (elapsed >= cooldown) {
          if (chatStatus.textContent !== 'Ready') {
              chatStatus.textContent = 'Ready';
              chatStatus.className = 'status-chip ready';
              chatBtn.disabled = false;
              chatBtn.textContent = 'Go to Chat';
              chatBtn.onclick = async () => {
                   // Open Telegram Link (mock)
                   if (window.Telegram && window.Telegram.WebApp) {
                       window.Telegram.WebApp.openTelegramLink('https://t.me/tgm_coin_chat');
                   }
                   currentState = await adapter.chatClaim();
                   render();
              };
          }
      } else {
          const left = cooldown - elapsed;
          chatStatus.textContent = formatTime(left);
          if (chatStatus.classList.contains('ready')) chatStatus.className = 'status-chip';
          chatBtn.disabled = true;
          chatBtn.textContent = 'Wait...';
      }
  }

  // Miner Card Logic (Only Update Text/State)
  ...
  if (minerStatus && minerBtn) {
      // Check current state vs UI state to minimize thrashing
      if (currentState.miner.status === 'idle') {
          if (minerStatus.textContent !== 'Idle') {
          ...
          if (left > 0) {
              ...
              if (!minerStatus.classList.contains('ready')) minerStatus.className = 'status-chip ready';
              if (minerBtn.textContent !== 'Mining...') {
                   minerBtn.textContent = 'Mining...';
                   minerBtn.disabled = true;
              }
          } else {
              if (minerStatus.textContent !== 'Done') {
  ...
  // --- HEAVY RENDERING (Only on Dirty) ---

  // Shop Render
  const shopList = document.getElementById('shop-list');
  if (shopList && currentState.shop && currentState.shop.items) {
      // Check if inventory changed or first load
      const inventoryHash = getHash(JSON.stringify(currentState.boosts.inventory) + currentState.isPremium);

      // If view is visible AND (dirty OR hash changed OR empty)
      if (window.location.hash.includes('shop') && (adapter.dirty || lastRenderedShopHash !== inventoryHash || shopList.children.length === 0)) {
          ...
          currentState.shop.items.forEach(item => {
              ...
              if (owned) {
                  ...
                  btn.onclick = async (e) => {
                      e.stopPropagation(); // Prevent list item click bubbling
          ...
          lastRenderedShopHash = inventoryHash;
      }
  }
  ...
  if (taskList) {
      const doneHash = getHash(JSON.stringify(currentState.tasks.doneList));

      if (window.location.hash.includes('tasks') && (adapter.dirty || lastRenderedTaskHash !== doneHash || taskList.children.length === 0)) {
          ...
          lastRenderedTaskHash = doneHash;
      }
  }

  // FRIENDS UI Update
  const friendsView = document.getElementById('friends');
  if (friendsView && window.location.hash.includes('friends') && adapter.dirty) {
  ...
  if (minerInfo && adapter.dirty) {
      minerInfo.textContent = "Passive income (x10 in Sponsor Chat)";
  }

  // Clear dirty flag after heavy render
  adapter.dirty = false;
}
